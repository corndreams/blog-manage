  ...
  <div class="comments">
    <el-card class="toolbar" shadow="never">
      <div class="left">
        ...
        <el-input v-model="articleId" placeholder="文章ID筛选" clearable style="width: 160px;" />
...
const filtered = computed(() => {
  ...
  return store.list.filter(c => !k || `${c.name}${c.content}`.toLowerCase().includes(k))
})

const onSelectionChange = (rows: any[]) => { selected.value = rows.map(r => r.id) }
const remove = async (row: any) => {
  await ElMessageBox.confirm(`删除该评论？`, '提示', { type: 'warning' })
...
const onSearch = async () => {
  const aid = articleId.value ? Number(articleId.value) : undefined
  await store.fetch({ page: 1, article_id: aid })
}
